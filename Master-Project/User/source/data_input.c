...
void ReadSystemPara()
{
    ...
    //pSystemParam->margin = pSystemParam->margin<0.1?0.1:pSystemParam->margin;
...
void Acquire_Data()
{
   ...
   // pData_ADC->adcvalue_valve = Get_Vx_adc(VA_CH);
    ...
    pData_Voltage->voltage_vcc  = 5.00;
...
void Update_InputCurrent()
{
    ...
    //cur_in =  * 16.0 /  + 4.0 - 16*(float)pSystemParam->setinput_low / (float)(pSystemParam->setinput_high-pSystemParam->setinput_low);
	...
	//printf("\r\n in_mA:%f  k=%f",cur_in,k);
	//cur_in = cur_in/1000.0;
	//printf("\r\n in_A:%f",cur_in);
