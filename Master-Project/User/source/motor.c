...
#define MOTOR_DELAY_CNT     (MOTOR_DELAY_MS / T1_MS)
#define TORQUE_DELAY_CNT    (TORQUE_DELAY_MS / 60)
#define ALARM_TIME_MS       10000 //10s
#define ALARM_TIME_CNT      (ALARM_TIME_MS / T1_MS)
...
//================================================================
u8 bdata Motor_CR;                          //Motor_CR
sbit  motordelay_flag   = Motor_CR^7;       //Bit7      Bit6      
...
void MotorOpen_Control()
{
    switch(motor_status)
    {
        ...
        {
            if(torquelock_open_flag != 1 && open100_flag !=1 && PIN_LIM_OPN != LIM_ON && motordelay_flag == 1)
            { 
...
void MotorClose_Control()
{
    switch(motor_status)
    {
        ...
        case MOTOR_STATUS_STOP:
        {
            if(torquelock_close_flag != 1 && close0_flag !=1 && PIN_LIM_CLE != LIM_ON && motordelay_flag == 1)//
            {
...
void MotorStop_Delay_60ms()//ç”µæœºåœæ­¢ä¿æŠ¤å»¶æ—¶ æ‰§è¡Œå‘¨æœŸï¼š50ms
{   
    if(motor_status == MOTOR_STATUS_STOP)
    {
        stop_cnt++;
...
void MotorErr_Detect()
{
	...
	if(PIN_ALARM == ALARM_ON && motor_status!=MOTOR_STATUS_STOP && pre_opening==opening)
	{
		motorerr_cnt++;
        if(motorerr_cnt>5) 
        {
...
//{
//    if(pData_Acquire->opening<0.002)
//    {
//        
//        if(motor_status == MOTOR_STATUS_CLOSE) 
//        {
...
//        
//    }
//    if(pData_Acquire->opening>=0.999)
//    {
//         if(motor_status == MOTOR_STATUS_OPEN)  
//         {
//             Motor_CR |= (u8)(1<<1); 
